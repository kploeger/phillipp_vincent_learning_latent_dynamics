...
class MovementGenerator:
    ...
    def __sin(self, amplitude, stretch, offset):
        return np.add(
            offset,
            np.multiply(amplitude, np.sin(np.multiply(stretch, self.inputs))),
        )

    def __van_der_pol(self, a, b, mu):
        def vdp(t, z):
            x, y = z
    ...
    def __sigmoid_activation_function(self, offset, amplitude):
        norm_arr = []
    ...
    def __normalize(self, t_min, t_max):
        norm_arr = []
        diff = t_max - t_min
    ...
    def generate(self):
        ...
        self.outputs = self.__normalize(-0.9, np.pi)
